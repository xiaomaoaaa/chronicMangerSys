    ...
    methods: {
        ...
        getData () {
            ...
            if (!this.async) {
                ...
                this.param.pageNo = this.pages[this.activeNum];
                this.param.pageSize = this.len;
                $.when(Public.commonajax(this.headersparam['X-Service-Id'],this.headersparam['X-Service-Method'],"["+JSON.stringify(this.param)+"]"))
                    .done(function(res){
                       if(res.body.total){
                         that.pageTotal = Math.ceil(res.body.total/10);
                       }else{
                        that.pageTotal=1;
                       }

                        if (that.pages.length !== that.pageLen || that.pageTotal < that.pageLen) {
                            that.getPages()
                        }

                        if (!res.body.list.length) {
                            that.activeNum = that.pageTotal - 1
                        }

                        that.$dispatch('data', res.body.list)
                ...
                //     dataType: "JSON",
                //     data:'[{"pageNo":'+that.pages[that.activeNum]+',"pageSize":'+that.len+'}]',
                //     beforeSend: function() {
                //        Public.
                //     },
